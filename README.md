...
The overlap workflow in [.github/workflows/daily-overlap-summary.yml](/Users/Zihao.Guan/Personal/ticker-screener/.github/workflows/daily-overlap-summary.yml) summarizes overlap across daily `RS`, `Sean PEG`, `Legacy PEG`, `VCP`, `Cup and Handle`, `Weekly HTF 8W Pullback`, and `8W 100% Runup`. It runs on its own schedule at `UTC 02:00`, which corresponds to New Zealand `2pm` during standard time and `3pm` during daylight saving because GitHub Actions schedules are UTC-based. It distinguishes missing upstream watchlist files from valid `0`-hit runs, writes JSON, text, and HTML summary artifacts, and follows the same R2 publish plus Discord notify pattern as the other first-class workflows.
