...
from src.vcs_screen import run_vcs_screen
from src.vcs_watchlist_builder import build_vcs_watchlist


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the VCS critical tightness screener.")
...
def main() -> int:
    ...
    result = run_vcs_screen(config, tickers, signal_profile="critical_tightness", as_of_date=as_of_date)
    watchlist = build_vcs_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="vcs_critical_tightness", date_label=date_label)
