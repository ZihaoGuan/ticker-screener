...
from src.vcp_scored_screen import WATCHLIST_MIN_SCORE_EXCLUSIVE, build_vcp_scored_result
from src.vcp_scored_watchlist_builder import build_vcp_scored_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run scored VCP screener.")
...
def main() -> int:
    ...
    _log(f"prepared vcp scored universe: total={len(tickers)} source={'manual-tickers' if args.tickers else 'database-universe'}")
    result = build_vcp_scored_result(config, tickers, as_of_date=as_of_date, database_url=database_url)
    watchlist_hits = [hit for hit in result.hits if hit.composite_score > WATCHLIST_MIN_SCORE_EXCLUSIVE]
    watchlist = build_vcp_scored_watchlist(watchlist_hits)
    _log(f"watchlist filter applied: raw_hits={len(result.hits)} scanner_entries_score_gt_80={len(watchlist_hits)}")

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="vcp_scored", date_label=date_label)
    ...
    _write_json(
        summary_path,
        {
            ...
            "watchlist_min_score_exclusive": WATCHLIST_MIN_SCORE_EXCLUSIVE,
