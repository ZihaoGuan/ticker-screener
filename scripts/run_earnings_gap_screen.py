...
from src.earnings_gap_watchlist_builder import build_earnings_gap_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run Pine-style PEG and Monster gap screeners.')
    ...
    parser.add_argument('--profile', choices=('peg', 'monster-gap', 'monster-peg'), default='peg')
...
def _strategy_id_for_profile(profile: str) -> str:
    if profile == 'monster-gap':
        return 'monster_gap'
    if profile == 'monster-peg':
        return 'monster_peg'
    return 'pine_peg'
...
def main() -> int:
    ...
    result = run_earnings_gap_screen(config, tickers, profile=args.profile, as_of_date=as_of_date)
    watchlist = build_earnings_gap_watchlist(result.hits, profile=args.profile)
    strategy_id = _strategy_id_for_profile(args.profile)
