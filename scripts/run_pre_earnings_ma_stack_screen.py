...
from src.pre_earnings_ma_stack_screen import run_pre_earnings_ma_stack_screen
from src.pre_earnings_ma_stack_watchlist_builder import build_pre_earnings_ma_stack_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the pre-earnings MA stack screener.")
...
def _next_week_events(
    ...
    universe = load_universe(config)
    sector_by_symbol = {item.symbol: (item.sector, item.exchange) for item in universe}
...
def main() -> int:
    ...
    result = run_pre_earnings_ma_stack_screen(config, events)
    watchlist = build_pre_earnings_ma_stack_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"pre_earnings_ma_stack_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"pre_earnings_ma_stack_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"pre_earnings_ma_stack_run_summary_{date_label}.json"
