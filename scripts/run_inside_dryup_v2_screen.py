...
from src.inside_dryup_v2_screen import run_inside_dryup_v2_screen
from src.inside_dryup_v2_watchlist_builder import build_inside_dryup_v2_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the inside-day + extreme dry-up v2 screen.")
...
def main() -> int:
    ...
    result = run_inside_dryup_v2_screen(config, universe, as_of_date=as_of_date)
    watchlist = build_inside_dryup_v2_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="inside_dryup_v2", date_label=date_label)
    ...
    _write_json(
        summary_path,
        {
            ...
            "signal_profile": "inside_dryup_v2",
