...
from src.ftd_sweep_screen import run_ftd_sweep_screen
from src.ftd_sweep_watchlist_builder import build_ftd_sweep_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Follow Through Day plus sweep breakout screen.")
...
def main() -> int:
    ...
    result = run_ftd_sweep_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_ftd_sweep_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"ftd_sweep_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"ftd_sweep_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"ftd_sweep_run_summary_{date_label}.json"
