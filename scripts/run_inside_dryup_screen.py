...
from src.inside_dryup_screen import run_inside_dryup_screen
from src.inside_dryup_watchlist_builder import build_inside_dryup_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the inside-day dry-up screener.")
...
def main() -> int:
    ...
    result = run_inside_dryup_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_inside_dryup_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"inside_dryup_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"inside_dryup_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"inside_dryup_run_summary_{date_label}.json"
