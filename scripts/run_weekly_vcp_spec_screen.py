...
from src.weekly_vcp_spec_screen import load_weekly_vcp_spec_universe, run_weekly_vcp_spec_screen
from src.weekly_vcp_spec_watchlist_builder import build_weekly_vcp_spec_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the weekly strict VCP spec screener.")
...
def main() -> int:
    ...
    if args.tickers:
        ...
        tickers = load_weekly_vcp_spec_universe(as_of_date=as_of_date, limit=args.limit, database_url=database_url)
        tickers = filter_universe_tickers(tickers, excluded)
        tickers = filter_universe_by_criteria(tickers, filter_criteria)

    _log(f"prepared weekly vcp spec universe: total={len(tickers)} source={'manual-tickers' if args.tickers else 'database-universe'}")
    result = run_weekly_vcp_spec_screen(config, tickers, as_of_date=as_of_date, database_url=database_url)
    watchlist = build_weekly_vcp_spec_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="weekly_vcp_spec", date_label=date_label)
    ...
    _log(f"Wrote raw results to {artifact_paths.raw_results_path}")
    _log(f"Wrote watchlist to {artifact_paths.watchlist_path}")
    _log(f"Wrote run summary to {artifact_paths.summary_path}")
