...
from src.canslim_v2_watchlist_builder import build_canslim_v2_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the CANSLIM V2 score screen.")
...
def main() -> int:
    ...
    _log(f"prepared canslim_v2 universe: total={len(tickers)} source={'manual-tickers' if args.tickers else 'database-universe'}")
    result = run_canslim_v2_screen(config, tickers, as_of_date=as_of_date, database_url=database_url)
    watchlist_hits = [hit for hit in result.hits if hit.composite_score > WATCHLIST_MIN_SCORE_EXCLUSIVE]
    watchlist = build_canslim_v2_watchlist(watchlist_hits)
    _log(f"watchlist filter applied: raw_hits={len(result.hits)} scanner_entries_score_gt_80={len(watchlist_hits)}")

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="canslim_v2", date_label=date_label)
