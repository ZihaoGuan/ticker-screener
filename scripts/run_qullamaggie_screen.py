...
from src.qullamaggie_screen import QULLAMAGGIE_STRATEGY_ID, load_qullamaggie_universe, run_qullamaggie_screen
from src.qullamaggie_watchlist_builder import build_qullamaggie_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Qullamaggie momentum and consolidation candidate screener.")
...
def main() -> int:
    ...
    criteria = build_filter_criteria_from_args(args)

    if args.tickers:
        tickers = [UniverseTicker(symbol=symbol) for symbol in filter_symbols(args.tickers, excluded)]
        source = "manual-tickers"
    else:
        tickers = load_qullamaggie_universe(as_of_date=as_of_date, limit=args.limit, database_url=database_url)
        tickers = filter_universe_tickers(tickers, excluded)
        tickers = filter_universe_by_criteria(tickers, criteria)
        source = "database-universe"

    print(f"prepared Qullamaggie universe: total={len(tickers)} source={source}", flush=True)
    result = run_qullamaggie_screen(config, tickers, as_of_date=as_of_date, database_url=database_url)
    watchlist = build_qullamaggie_watchlist(result.hits)
    paths = build_screener_artifact_paths(
        PROJECT_ROOT / "artifacts",
        strategy_id=QULLAMAGGIE_STRATEGY_ID,
    ...
    _write_json(
        paths.summary_path,
        {
            ...
            "raw_results_file": str(paths.raw_results_path),
            "watchlist_file": str(paths.watchlist_path),
        },
    )
    print(f"Wrote raw results to {paths.raw_results_path}", flush=True)
    print(f"Wrote watchlist to {paths.watchlist_path}", flush=True)
    print(f"Wrote run summary to {paths.summary_path}", flush=True)
