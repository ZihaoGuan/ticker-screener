...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the potential overhead gap-fill screen.")
...
def main() -> int:
    ...
    result = run_gap_fill_screen(config, tickers)
    watchlist = build_gap_fill_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"gap_fill_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"gap_fill_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"gap_fill_run_summary_{date_label}.json"
