...
from src.position_action_daily_screen import run_position_action_daily_screen
from src.position_action_daily_watchlist_builder import build_position_action_daily_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the daily position action screen.")
    ...
    parser.add_argument(
        ...
        help="Prefer Postgres daily_bars and fall back to internet batches, or force internet mode.",
    )
...
def main() -> int:
    ...
    result = run_position_action_daily_screen(
        config,
        universe,
        as_of_date=as_of_date,
        database_url=resolve_database_url(web_config.database_url),
        market_data_source=args.market_data_source,
    )
    watchlist = build_position_action_daily_watchlist(result.hits)

    repository = PositionDecisionRepository(database_url=web_config.database_url)
    repository.upsert_daily_decisions(result.decision_rows)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="position_action_daily", date_label=date_label)
    ...
    _write_json(
        summary_path,
        {
            "strategy_id": "position_action_daily",
            "date_label": date_label,
            "as_of_date": as_of_date.isoformat() if as_of_date else None,
            "signal_profile": "position_action_daily",
            "market_data_source": args.market_data_source,
            "total_tickers": result.total_tickers,
            "passed_tickers": result.passed_tickers,
            "failed_tickers": result.failed_tickers,
            "decision_row_count": len(result.decision_rows),
    ...
    print(f"Upserted {len(result.decision_rows)} daily position decision row(s)")
    return 0
