...
from src.cup_handle_watchlist_builder import build_cup_handle_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Cup & Handle screener.")
...
def main() -> int:
    ...
    result = run_cup_handle_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_cup_handle_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"cup_handle_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"cup_handle_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"cup_handle_run_summary_{date_label}.json"
