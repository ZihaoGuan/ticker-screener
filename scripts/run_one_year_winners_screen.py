...
from src.one_year_winners_screen import (
    ONE_YEAR_WINNERS_STRATEGY_ID,
    load_one_year_winners_universe,
    run_one_year_winners_screen,
)
from src.one_year_winners_watchlist_builder import build_one_year_winners_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the rolling one-year winners above $10B screener.")
...
def main() -> int:
    ...
    if args.tickers:
        ...
        tickers = load_one_year_winners_universe(
    ...
    print(f"prepared one-year winners universe: total={len(tickers)} source={source}", flush=True)
    ...
    watchlist = build_one_year_winners_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(
        PROJECT_ROOT / "artifacts",
        strategy_id=ONE_YEAR_WINNERS_STRATEGY_ID,
