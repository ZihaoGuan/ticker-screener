...
from src.weekly_vcp_v3_screen import run_weekly_vcp_v3_screen
from src.weekly_vcp_v3_watchlist_builder import build_weekly_vcp_v3_watchlist


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the weekly VCP v3 screener.")
...
def main() -> int:
    ...
    result = run_weekly_vcp_v3_screen(config, tickers, as_of_date=as_of_date, database_url=database_url)
    watchlist = build_weekly_vcp_v3_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="weekly_vcp_v3", date_label=date_label)
