...
from src.weekly_candidate_pool_screen import WEEKLY_CANDIDATE_POOL_STRATEGY_ID, run_weekly_candidate_pool_screen
from src.weekly_candidate_pool_watchlist_builder import build_weekly_candidate_pool_watchlist


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the weekly candidate-pool screener.")
...
def main() -> int:
    ...
    if args.tickers:
        ...
        tickers = filter_universe_by_criteria(load_universe(config, limit=args.limit), build_filter_criteria_from_args(args))

    result = run_weekly_candidate_pool_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_weekly_candidate_pool_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id=WEEKLY_CANDIDATE_POOL_STRATEGY_ID, date_label=date_label)
    ...
    _write_json(
        ...
        {
            "strategy_id": WEEKLY_CANDIDATE_POOL_STRATEGY_ID,
