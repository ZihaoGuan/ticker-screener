...
from src.rs_phase_screen import RS_PHASE_MIN_ACTIVE_DAYS, run_rs_phase_screen
from src.rs_phase_watchlist_builder import build_rs_phase_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the RS Phase screen.")
    ...
    parser.add_argument("--min-active-days", type=int, default=RS_PHASE_MIN_ACTIVE_DAYS)
...
def main() -> int:
    ...
    result = run_rs_phase_screen(config, universe, as_of_date=as_of_date, min_active_days=args.min_active_days)
    watchlist = build_rs_phase_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="rs_phase", date_label=date_label)
