...
from src.wyckoff_analysis import run_wyckoff_signal_screen
from src.wyckoff_signal_watchlist_builder import build_wyckoff_signal_watchlist


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Wyckoff sell signal screen.")
...
def main() -> int:
    ...
    result = run_wyckoff_signal_screen(config, universe, signal_type="sell", as_of_date=as_of_date)
    watchlist = build_wyckoff_signal_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="wyckoff_sell_signal", date_label=date_label)
    ...
    _write_json(
        summary_path,
        {
            ...
            "signal_profile": "wyckoff_sell_signal",
            "signal_type": "sell",
