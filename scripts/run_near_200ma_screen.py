...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the near-200D moving average screen.")
...
def main() -> int:
    ...
    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"near_200ma_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"near_200ma_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"near_200ma_run_summary_{date_label}.json"
    ...
    _write_json(
        summary_path,
        {
            ...
            "bull_count": sum(1 for item in result.hits if item.case_group == "bull"),
            "bear_count": sum(1 for item in result.hits if item.case_group == "bear"),
