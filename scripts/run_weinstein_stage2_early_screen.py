...
from src.weinstein_stage2_early_screen import run_weinstein_stage2_early_screen
from src.weinstein_stage2_early_watchlist_builder import build_weinstein_stage2_early_watchlist


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Weinstein Stage 2 Early screener.")
...
def main() -> int:
    ...
    result = run_weinstein_stage2_early_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_weinstein_stage2_early_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="weinstein_stage2_early", date_label=date_label)
