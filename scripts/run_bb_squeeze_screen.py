...
from src.bb_squeeze_screen import run_bb_squeeze_screen
from src.bb_squeeze_watchlist_builder import build_bb_squeeze_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BB squeeze screen.")
...
def main() -> int:
    ...
    result = run_bb_squeeze_screen(config, universe, as_of_date=as_of_date)
    watchlist = build_bb_squeeze_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="bb_squeeze", date_label=date_label)
