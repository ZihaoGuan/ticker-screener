...
from src.market_data_access import load_ticker_metadata_map
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch Finviz IPO dates and cache them in ticker_metadata.")
    ...
    parser.add_argument("--delay-min-seconds", type=float, default=0.0)
    parser.add_argument("--delay-max-seconds", type=float, default=0.0)
    parser.add_argument("--batch-size-before-rest", type=int, default=0)
...
def _manifest_path(args: argparse.Namespace) -> Path:
    ...
    return PROJECT_ROOT / "artifacts" / "raw" / "finviz_ipo_dates_manifest.json"
...
def main() -> int:
    ...
    existing_metadata = load_ticker_metadata_map([item.symbol for item in universe], database_url=database_url)

    print(
        "ipo_date_sync_config "
    ...
    for index, ticker_meta in enumerate(universe, start=1):
        ...
        if is_known_missing_finviz_ticker(ticker, artifacts_dir=artifacts_dir):
            ...
            if _should_write_manifest(index, len(universe)):
                _write_manifest(manifest_path, args=args, completed=completed, failed=failed, blocked=blocked, next_resume_ticker=next_resume)
            continue

        existing_ipo_date = str((existing_metadata.get(ticker) or {}).get("ipo_date") or "").strip()
        if args.overwrite_policy == "skip-existing" and existing_ipo_date:
            completed.append(ticker)
            print(f"[{index}/{len(universe)}] {ticker} skipped_existing ipo_date={existing_ipo_date}", flush=True)
            next_resume = universe[index].symbol if index < len(universe) else None
            if _should_write_manifest(index, len(universe)):
                _write_manifest(manifest_path, args=args, completed=completed, failed=failed, blocked=blocked, next_resume_ticker=next_resume)
            continue
        ...
        try:
            ...
            if snapshot_needs_fallback(snapshot) or snapshot.ipo_date is None:
                raise FinvizApiError("API snapshot missing IPO date or classification.")
        except FinvizApiError as exc:
            failure_message = str(exc)
            if finviz_error_is_missing(exc):
                record_missing_finviz_ticker(ticker, reason=str(exc), artifacts_dir=artifacts_dir)
            for attempt in range(1, 4):
                try:
                    ...
                    if finviz_probe_is_missing(probe):
                        record_missing_finviz_ticker(
                            ticker,
                            reason=f"HTTP {probe.status_code or 'unknown'}",
                            artifacts_dir=artifacts_dir,
                        )
                        failure_message = f"Finviz missing ticker: HTTP {probe.status_code or 'unknown'}"
                        break
                    ...
                    if snapshot.ipo_date is None:
                        failure_message = "Finviz probe returned no IPO date."
                    break
                except FinvizProbeError as probe_exc:
                    if failure_message:
                        failure_message = f"{failure_message}; fallback_probe={probe_exc}"
                    else:
                        failure_message = str(probe_exc)
                    if looks_retryable_failure(failure_message) and attempt < 3:
                        time.sleep(20.0 if attempt == 1 else 60.0)
                        continue
                    break

        if snapshot is None or snapshot.ipo_date is None:
            failed.append({"ticker": ticker, "reason": failure_message or "missing_ipo_date"})
            print(f"[{index}/{len(universe)}] {ticker} ipo_date_failed reason={failure_message or 'missing_ipo_date'}", flush=True)
            if "unexpected http status: 503" in str(failure_message).lower():
                consecutive_503 += 1
            else:
                consecutive_503 = 0
        else:
            ...
            repository.upsert_ticker_metadata_ipo_dates([(ticker, snapshot.ipo_date, "finviz-ipo-date")])
            existing_metadata[ticker] = {
                **(existing_metadata.get(ticker) or {}),
                "ipo_date": snapshot.ipo_date.isoformat(),
            }
            completed.append(ticker)
            consecutive_blocked = 0
            consecutive_503 = 0
            print(f"[{index}/{len(universe)}] {ticker} ipo_date_ok ipo_date={snapshot.ipo_date.isoformat()}", flush=True)

        next_resume = universe[index].symbol if index < len(universe) else None
        if _should_write_manifest(index, len(universe), force=blocked_this_ticker or snapshot is None or snapshot.ipo_date is None):
            _write_manifest(manifest_path, args=args, completed=completed, failed=failed, blocked=blocked, next_resume_ticker=next_resume)
    ...
    print(f"ipo_dates_completed={len(completed)}", flush=True)
    print(f"ipo_dates_failed={len(failed)}", flush=True)
    _write_manifest(manifest_path, args=args, completed=completed, failed=failed, blocked=blocked, next_resume_ticker=None)
    return 0
