...
from src.vcp_spec_screen import load_vcp_spec_universe, run_vcp_spec_screen
from src.vcp_spec_watchlist_builder import build_vcp_spec_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the strict VCP spec screener.")
...
def main() -> int:
    ...
    if args.tickers:
        ...
        tickers = load_vcp_spec_universe(
    ...
    _log(
        f"prepared vcp spec universe: total={len(tickers)} "
        f"source={'manual-tickers' if args.tickers else 'database-universe'}"
    )
    result = run_vcp_spec_screen(
        config,
        tickers,
        as_of_date=as_of_date,
        database_url=database_url,
    )
    watchlist = build_vcp_spec_watchlist(result.hits)
