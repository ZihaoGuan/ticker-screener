...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run live Finviz-backed Venu scanner.")
...
def main() -> int:
    ...
    result = run_venu_scanner(limit=args.limit, tickers=requested_tickers)
    watchlist = list(result.get("hits") or [])

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="venu_scanner", date_label=date_label)
