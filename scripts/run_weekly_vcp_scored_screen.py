...
from src.weekly_vcp_scored_screen import build_weekly_vcp_scored_result
from src.weekly_vcp_scored_watchlist_builder import build_weekly_vcp_scored_watchlist


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the weekly VCP scored screener.")
...
def main() -> int:
    ...
    result = build_weekly_vcp_scored_result(config, tickers, as_of_date=as_of_date, database_url=database_url)
    watchlist = build_weekly_vcp_scored_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="weekly_vcp_scored", date_label=date_label)
    ...
    _write_json(
        artifact_paths.summary_path,
        {
            ...
            "watchlist_passed_tickers": result.watchlist_passed_tickers,
