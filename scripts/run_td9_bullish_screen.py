...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the bullish TD Sequential 9 screener.")
...
def main() -> int:
    ...
    result = run_td_sequential_screen(config, tickers, direction="bullish", as_of_date=as_of_date)
    watchlist = build_td_sequential_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="td9_bullish", date_label=date_label)
