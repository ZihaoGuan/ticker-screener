...
from src.elite_rs_watchlist_builder import build_elite_rs_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run elite RS volume-signature screener.')
    ...
    parser.add_argument('--profile', choices=('hv1', 'recent-peg'), default='hv1')
...
def _strategy_id_for_profile(profile: str) -> str:
    return 'elite_rs_recent_peg' if profile == 'recent-peg' else 'elite_rs_hv1'
...
def main() -> int:
    ...
    result = run_elite_rs_screen(config, tickers, profile=args.profile, as_of_date=as_of_date)
    watchlist = build_elite_rs_watchlist(result.hits, profile=args.profile)
