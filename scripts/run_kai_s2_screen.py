...
from src.kai_s2_screen import KAI_S2_STRATEGY_ID, load_kai_s2_universe, run_kai_s2_screen
from src.kai_s2_watchlist_builder import build_kai_s2_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Kai S2 screener.")
...
def main() -> int:
    ...
    if args.tickers:
        ...
        tickers = load_kai_s2_universe(as_of_date=as_of_date, limit=args.limit, database_url=database_url)
    ...
    print(f"prepared kai s2 universe: total={len(tickers)} source={source}", flush=True)
    result = run_kai_s2_screen(config, tickers, as_of_date=as_of_date, database_url=database_url)
    watchlist = build_kai_s2_watchlist(result.hits)
    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id=KAI_S2_STRATEGY_ID, date_label=date_label)
    ...
    _write_json(
        ...
        {
            "strategy_id": KAI_S2_STRATEGY_ID,
    ...
    print(f"Wrote raw results to {artifact_paths.raw_results_path}", flush=True)
    print(f"Wrote watchlist to {artifact_paths.watchlist_path}", flush=True)
    print(f"Wrote run summary to {artifact_paths.summary_path}", flush=True)
