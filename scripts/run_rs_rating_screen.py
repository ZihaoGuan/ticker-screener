...
from src.rs_rating_screen import run_rs_rating_screen
from src.rs_rating_watchlist_builder import build_rs_rating_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the RS rating screen.")
...
def main() -> int:
    ...
    result = run_rs_rating_screen(config, universe, as_of_date=as_of_date)
    watchlist = build_rs_rating_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="rs_rating", date_label=date_label)
    ...
    _write_json(
        summary_path,
        {
            "strategy_id": "rs_rating",
            "date_label": date_label,
            "as_of_date": as_of_date.isoformat() if as_of_date else None,
            "signal_profile": "latest_rating",
