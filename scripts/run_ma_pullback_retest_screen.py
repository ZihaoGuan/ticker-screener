...
from src.ma_pullback_retest_screen import MA_PULLBACK_RETEST_STRATEGY_ID, run_ma_pullback_retest_screen
from src.ma_pullback_retest_watchlist_builder import build_ma_pullback_retest_watchlist
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the MA Pullback & Retest family screener.")
    ...
    result = run_ma_pullback_retest_screen(config, tickers, as_of_date=as_of_date)
    date_label = args.date_label or today_label(as_of_date)
    paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id=MA_PULLBACK_RETEST_STRATEGY_ID, date_label=date_label)
    ...
    paths.watchlist_path.write_text(json.dumps(build_ma_pullback_retest_watchlist(result.hits), indent=2), encoding="utf-8")
    paths.summary_path.write_text(json.dumps({"strategy_id": MA_PULLBACK_RETEST_STRATEGY_ID, "date_label": date_label, "as_of_date": as_of_date.isoformat() if as_of_date else None, "total_tickers": result.total_tickers, "passed_tickers": result.passed_tickers, "failed_tickers": result.failed_tickers, "raw_results_file": str(paths.raw_results_path), "watchlist_file": str(paths.watchlist_path)}, indent=2), encoding="utf-8")
    persisted = persist_screen_run_artifacts_if_configured(args=args, summary_path=paths.summary_path)
    print(f"MA Pullback & Retest: {result.passed_tickers}/{result.total_tickers}", flush=True)
