...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the daily RS new-high screen.")
...
def main() -> int:
    ...
    config = override_config(config, rs_new_high_require_before_price=False)
    if args.limit:
    ...
    result = run_rs_screen(config, universe, as_of_date=as_of_date, require_before_price=False)
    watchlist = build_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="daily_rs_new_high", date_label=date_label)
    ...
    _write_json(
        summary_path,
        {
            ...
            "signal_profile": "daily",
            "require_before_price": False,
