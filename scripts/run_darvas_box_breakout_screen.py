...
from src.darvas_box_screen import run_darvas_box_breakout_screen
from src.darvas_box_watchlist_builder import build_darvas_box_breakout_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Darvas box breakout screener.")
...
def main() -> int:
    ...
    tickers = (
        [UniverseTicker(symbol=symbol) for symbol in filter_symbols(args.tickers, excluded)]
        if args.tickers
        else filter_universe_by_criteria(load_universe(config, limit=args.limit), criteria)
    )
    result = run_darvas_box_breakout_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_darvas_box_breakout_watchlist(result.hits)
    paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="darvas_box_breakout", date_label=date_label)
    ...
    _write_json(paths.summary_path, {
        "strategy_id": "darvas_box_breakout", "date_label": date_label,
        "as_of_date": as_of_date.isoformat() if as_of_date else None,
        "total_tickers": result.total_tickers, "passed_tickers": result.passed_tickers,
        "failed_tickers": result.failed_tickers, "raw_results_file": str(paths.raw_results_path),
