...
from src.fearzone_screen import run_fearzone_screen
from src.fearzone_watchlist_builder import build_fearzone_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Fearzone buy screener.")
...
def main() -> int:
    ...
    result = run_fearzone_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_fearzone_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"fearzone_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"fearzone_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"fearzone_run_summary_{date_label}.json"
