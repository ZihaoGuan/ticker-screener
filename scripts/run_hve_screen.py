...
from src.hve_screen import run_hve_screen
from src.hve_watchlist_builder import build_hve_watchlist
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the HVE all-time highest-volume screen.")
...
def main() -> int:
    ...
    result = run_hve_screen(config, universe, as_of_date=as_of_date)
    watchlist = build_hve_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"hve_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"hve_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"hve_run_summary_{date_label}.json"
