...
def main() -> int:
    ...
    result = run_htf_runup_screen(config, universe)
    watchlist = build_htf_runup_watchlist(result.hits)

    raw_path = PROJECT_ROOT / "artifacts" / "raw" / f"htf_8w_runup_{date_label}.json"
    watchlist_path = PROJECT_ROOT / "artifacts" / "watchlists" / f"htf_8w_runup_{date_label}.json"
    summary_path = PROJECT_ROOT / "artifacts" / "raw" / f"htf_8w_runup_run_summary_{date_label}.json"
