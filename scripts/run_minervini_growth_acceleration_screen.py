...
from src.minervini_growth_acceleration_screen import run_minervini_growth_acceleration_screen
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Minervini growth acceleration screener.")
...
def main() -> int:
    ...
    result = run_minervini_growth_acceleration_screen(tickers, as_of_date=as_of_date)
    watchlist = [
        {
            **item.to_dict(),
            "growth_acceleration_score": item.acceleration_score,
            "growth_acceleration_label": item.acceleration_label,
        }
        for item in result.hits
    ]

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="minervini_growth_acceleration", date_label=date_label)
