...
from src.vcp_v3_screen import run_vcp_v3_screen
from src.vcp_v3_watchlist_builder import build_vcp_v3_watchlist


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the VCP v3 screener.")
...
def main() -> int:
    ...
    result = run_vcp_v3_screen(config, tickers, as_of_date=as_of_date)
    watchlist = build_vcp_v3_watchlist(result.hits)

    artifact_paths = build_screener_artifact_paths(PROJECT_ROOT / "artifacts", strategy_id="vcp_v3", date_label=date_label)
