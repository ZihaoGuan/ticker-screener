...
class AdHocScreenServiceTests(unittest.TestCase):
    ...
    def test_run_supports_high_tight_flag_setup_catalog_entry(self) -> None:
        ticker_frame = _high_tight_flag_setup_frame(passes=True)
        benchmark_frame = _frame("2026-01-02", 260)
        service = AdHocScreenService(app_config=AppConfig(), database_url="postgres://unit-test")
        ...
        with patch(
            ...
            payload = service.run(
                ticker="AAPL",
                as_of_date=dt.date(2025, 12, 31),
                screener_ids=["high_tight_flag_setup"],
            )

        self.assertEqual(payload["summary"]["passed_screener_count"], 1)
        self.assertEqual(payload["screeners"][0]["id"], "high_tight_flag_setup")
        self.assertGreaterEqual(payload["screeners"][0]["hit"]["pole_gain_ratio"], 1.9)
        self.assertLess(payload["screeners"][0]["hit"]["distance_to_pivot_pct"], 0.08)
        self.assertTrue(payload["screeners"][0]["passed"])
