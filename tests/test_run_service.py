...
class RunServiceTests(unittest.TestCase):
    ...
    def test_list_actions_includes_finviz_target_price_scanner(self) -> None:
        actions = {item["id"]: item for item in self.service.list_actions()}

        self.assertIn("finviz_target_price_50", actions)
    ...
    def test_list_actions_includes_near_52wk_high(self) -> None:
        ...
        self.assertIn("near_52wk_high", actions)
        self.assertEqual(actions["near_52wk_high"]["label"], "Run Near 52W High")
        self.assertIn("scripts/run_near_52wk_high_screen.py", actions["near_52wk_high"]["command"])
    ...
    def test_build_command_supports_split_history_refresh(self) -> None:
        command = self.service.build_command(
            ...
            {"as_of_date": "2026-09-19", "start_date": "2020-01-01", "lookback_days": 90},
        )

        self.assertEqual(command[1], "scripts/refresh_split_adjusted_history.py")
        self.assertEqual(
            command[-6:],
            ["--as-of-date", "2026-09-19", "--lookback-days", "90", "--start-date", "2020-01-01"],
        )
