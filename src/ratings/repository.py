    ...
    def upsert_fundamentals_snapshots(self, snapshots: Iterable[FundamentalsSnapshot]) -> int:
        ...
        sql = """
            INSERT INTO ticker_fundamentals_snapshots (
              ...
              operating_margin_pct, gross_margin_pct, roa_pct, roe_pct, roic_pct, institutional_ownership_pct,
              institutional_transactions_pct, insider_ownership_pct, insider_transactions_pct,
              shares_float, shares_outstanding, current_ratio, quick_ratio, debt_to_equity, lt_debt_to_equity, eps_next_q, eps_this_y_pct,
              eps_next_y_pct, eps_next_5y_pct, sales_qq_pct, sales_yoy_ttm_pct, eps_qq_pct, eps_yoy_ttm_pct,
              eps_surprise_pct, sales_surprise_pct, analyst_recommendation, target_price, perf_month_pct,
            ...
            ON CONFLICT (ticker, as_of_date) DO UPDATE SET
              ...
              institutional_transactions_pct = EXCLUDED.institutional_transactions_pct,
              insider_ownership_pct = EXCLUDED.insider_ownership_pct,
              insider_transactions_pct = EXCLUDED.insider_transactions_pct,
              ...
              eps_next_q = EXCLUDED.eps_next_q,
              ...
              sales_yoy_ttm_pct = EXCLUDED.sales_yoy_ttm_pct,
              eps_qq_pct = EXCLUDED.eps_qq_pct,
              eps_yoy_ttm_pct = EXCLUDED.eps_yoy_ttm_pct,
              eps_surprise_pct = EXCLUDED.eps_surprise_pct,
              sales_surprise_pct = EXCLUDED.sales_surprise_pct,
        ...
        payload = [
            (
                ...
                item.institutional_transactions_pct,
                item.insider_ownership_pct,
                item.insider_transactions_pct,
                item.shares_float,
                item.shares_outstanding,
                item.current_ratio,
                item.quick_ratio,
                item.debt_to_equity,
                item.lt_debt_to_equity,
                item.eps_next_q,
                item.eps_this_y_pct,
                item.eps_next_y_pct,
                item.eps_next_5y_pct,
                item.sales_qq_pct,
                item.sales_yoy_ttm_pct,
                item.eps_qq_pct,
                item.eps_yoy_ttm_pct,
                item.eps_surprise_pct,
                item.sales_surprise_pct,
                item.analyst_recommendation,
                item.target_price,
                item.perf_month_pct,
                    ...
                    cursor.executemany(
                        ...
                        [
                            (
                                ...
                                item.industry_group,
                                item.industry_group_rs_rank,
                                item.industry_group_member_count,
                                item.overall_rating,
                                item.rating_band,
        ...
        *,
        ...
        sql = """
            SELECT fs.ticker, fs.as_of_date, COALESCE(fs.sector, tm.sector) AS sector,
                   ...
                   fs.institutional_transactions_pct, fs.insider_ownership_pct, fs.insider_transactions_pct,
                   fs.shares_float, fs.shares_outstanding, fs.eps_this_y_pct, fs.eps_next_y_pct, fs.eps_next_5y_pct,
    ...
    def load_latest_ticker_rating_bundle(self, ticker: str) -> dict[str, Any] | None:
        ...
        fundamentals_sql = """
            SELECT
              ...
              f.institutional_transactions_pct,
              f.insider_ownership_pct,
              f.insider_transactions_pct,
              f.shares_float,
        ...
        if fundamentals_row:
            ...
            fundamentals_snapshot = {
                ...
                "roic_pct": float(roic_pct) if roic_pct is not None else None,
                "institutional_ownership_pct": float(institutional_ownership_pct) if institutional_ownership_pct is not None else None,
                "institutional_transactions_pct": float(institutional_transactions_pct) if institutional_transactions_pct is not None else None,
                "insider_ownership_pct": float(insider_ownership_pct) if insider_ownership_pct is not None else None,
                "insider_transactions_pct": float(insider_transactions_pct) if insider_transactions_pct is not None else None,
                ...
                "lt_debt_to_equity": float(lt_debt_to_equity) if lt_debt_to_equity is not None else None,
                "eps_next_q": float(eps_next_q) if eps_next_q is not None else None,
                ...
                "sales_yoy_ttm_pct": float(sales_yoy_ttm_pct) if sales_yoy_ttm_pct is not None else None,
                "eps_qq_pct": float(eps_qq_pct) if eps_qq_pct is not None else None,
                "eps_yoy_ttm_pct": float(eps_yoy_ttm_pct) if eps_yoy_ttm_pct is not None else None,
                "eps_surprise_pct": float(eps_surprise_pct) if eps_surprise_pct is not None else None,
                "sales_surprise_pct": float(sales_surprise_pct) if sales_surprise_pct is not None else None,
                "analyst_recommendation": float(analyst_recommendation) if analyst_recommendation is not None else None,
        ...
        *,
        ...
        for (
            ...
            result[str(ticker).upper()] = {
                ...
                "industry_group_member_count": int(industry_group_member_count) if industry_group_member_count is not None else None,
